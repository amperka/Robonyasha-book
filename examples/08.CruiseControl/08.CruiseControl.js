...
(22-27) Смысл этого кода такой: «Если текущая скорость меньше круизной и напряжение меньше 100%, увеличь напряжение на моторе. Если скорость больше круизной и напряжение больше нуля, уменьшай напряжение.» Функция Math.abs() возвращает модуль числа (убирает минус).
(31-32) Вызываем cruise() для обоих моторов по очереди и передаём в скобках значения параметров. Чтобы моторы крутились в одну сторону, одному из них нужно задать отрицательные STARTV и DELTAV. Если с этим кодом колёса будут крутиться назад, перенеси минусы в вызов функции для leftMotor.
*/
