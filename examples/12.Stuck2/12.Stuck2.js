...
(23) Функция ping() объекта ultrasonic измеряет расстояние до объекта. Результат измерения передаётся в функцию в виде переменной value. Единицы измерения расстояния указываются во втором аргументе функции ping(). Мы сейчас использовали сантиметры, но также есть и другие величины: миллиметры и метры. 
(24) Иногда произвести измерения не удаётся. Например, расстояние до предмета слишком велико или измерения выполняются слишком часто. В этом случае переменная error не будет пустой, а будет содержать информацию о характере ошибки. После каждого измерения всегда проверяй error на наличие ошибок.
*/
